...
# ============================================================
#  NightEnemySpawner
#  Spawne des ennemis nocturnes uniquement pendant la nuit.
#  Ils disparaissent (fade out + queue_free) à l'aube.
#
#  🔧 AJOUTER UN ENNEMI NOCTURNE :
#  Dans NIGHT_ENEMY_DEFS, ajoute un dictionnaire :
#  {
#      "scene":  "res://scenes/MON_ENNEMI.tscn",   ← scène Godot
#      "max":    3,                                 ← nb max simultané
...
# ── ✏️  CONFIGURATION DES ENNEMIS NOCTURNES ─────────────────
const NIGHT_ENEMY_DEFS: Array = [
	...
	# ── Exemple : ajouter un zombie (à décommenter quand la scène existe)
	# {
	#     "scene":     "res://scenes/zombie.tscn",
	#     "max":       4,
	#     "overrides": { "speed": 40.0, "max_health": 80 }
	# },
]
...
# ── Terrain valide pour spawner : herbe + chemin ─────────────
...
func _ready() -> void:
	...
	if not _terrain_layer:
		push_error("NightEnemySpawner : TileMapLayer 'Terrains' introuvable !")
		return
	_scan_tiles()
	...
	if not day_night:
		push_error("NightEnemySpawner : nœud 'day_night' introuvable (groupe manquant) !")
		return
	day_night.time_of_day_changed.connect(_on_time_of_day_changed)

	# État initial : si on démarre la nuit, spawner immédiatement
	if day_night.is_night():
		_on_time_of_day_changed("night")
...
# ============================================================
#  SPAWN INITIAL DE NUIT
# ============================================================

func _spawn_all_night_enemies() -> void:
	for def in NIGHT_ENEMY_DEFS:
...
func _try_spawn(def: Dictionary) -> void:
	...
	if not scene_res:
		push_error("NightEnemySpawner : scène introuvable — " + def["scene"])
		return
...
func _fade_and_free(node: Node) -> void:
	# Désactiver les collisions immédiatement pour qu'il ne gêne plus
