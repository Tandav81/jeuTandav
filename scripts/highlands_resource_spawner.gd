...
# ────────────────────────────────────────────────────────────
var _highlands_positions:  Array = []
...
func _ready() -> void:
	...
	if not _terrain_layer:
		push_error("HighlandsResourceSpawner: TileMapLayer 'Terrains' introuvable !")
		return
	_scan_tiles()
	if _highlands_positions.is_empty():
		push_error("HighlandsResourceSpawner: aucune tile source_id=13 atlas=(2,2) trouvée !")
		return
	_initial_spawn()
...
func _scan_tiles() -> void:
	...
	for cell in _terrain_layer.get_used_cells():
		...
		if src == HIGHLANDS_SOURCE_ID and coords == HIGHLANDS_ATLAS_COORDS:
			...
			_highlands_positions.append(local_px * tm_scale + tm_offset)

	_highlands_positions.shuffle()
...
func _pick_free_position() -> Vector2:
	for _i in range(60):
		var candidate: Vector2 = _highlands_positions[randi() % _highlands_positions.size()]
		var ok = true
